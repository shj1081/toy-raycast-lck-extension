...
export default function Command() {
  ...
  React.useEffect(() => {
    ...
    // 두 달치 데이터 모두 가져오기
    Promise.all([
      fetch(`https://esports-api.game.naver.com/service/v2/schedule/month?month=${currentMonth}&topLeagueId=lck&relay=false`)
        .then(res => res.json()) as Promise<ApiResponse>,
      fetch(`https://esports-api.game.naver.com/service/v2/schedule/month?month=${nextMonth}&topLeagueId=lck&relay=false`)
        .then(res => res.json()) as Promise<ApiResponse>
    ])
      .then(([currentMonthData, nextMonthData]) => {
        ...
        const filteredMatches = allMatches.filter(match => 
          ...
          match.homeTeam.nameAcronym !== "TBD" && match.awayTeam.nameAcronym !== "TBD"
        );
  ...
  // 가장 빠른 날짜에 해당하는 경기들만 추출
  const nextDayMatches = nextMatchDate ? upcomingMatches.filter(match => formatDate(match.startDate) === nextMatchDate) : [];
  ...
  const filteredPastDateKeys = limitedPastDateKeys.filter(
    dateKey => getFilteredPastMatches(dateKey).length > 0
  );
  
  const filteredUpcomingDateKeys = sortedDateKeys.filter(
    dateKey => getFilteredUpcomingMatches(dateKey).length > 0
  );
  
  return (
    <List 
      isLoading={isLoading} 
      searchBarPlaceholder="팀명 또는 일정 검색"
            ...
            return (
              <List.Item
                ...
                accessories={[
                  ...
                  ...(scoreLabel ? [{ tag: { value: `${match.homeScore} : ${match.awayScore}`, color: Color.Green } }] : [])
                ]}
              />
            );
          })}
